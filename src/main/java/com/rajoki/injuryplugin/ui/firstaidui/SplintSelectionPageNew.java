...
public class SplintSelectionPageNew extends InteractiveCustomUIPage<SplintSelectionPageNew.SplintEventData> {

    private static final ScheduledExecutorService scheduler;

    static {
        scheduler = Executors.newScheduledThreadPool(1);
        //System.out.println("[DEBUG] SplintPageNew scheduler created: " + scheduler);
    }
    private ScheduledFuture<?> refreshTask;
    private final PlayerRef playerRef;

    public SplintSelectionPageNew(@Nonnull PlayerRef playerRef) {
        super(playerRef, CustomPageLifetime.CanDismissOrCloseThroughInteraction, SplintEventData.CODEC);
    ...
    public void build(@Nonnull Ref<EntityStore> ref,
        ...
        SplintSelectionUIRegistry.register(playerRef, this);

        uiCommandBuilder.append("SplintSelectionNew.ui");
        ...
        if (bodyPartData != null) {
            ...
            // NEW: Bind button events for splinting
            bindSplintButtons(uiEventBuilder, bodyPartData);
    ...
    private void bindSplintButtons(UIEventBuilder eventBuilder, BodyPartComponent bodyPartData) {
        // Add buttons for each body part with fractures
        for (BodyPart part : UI_PARTS) {
            boolean hasFracture = bodyPartData.hasBodyPartEffect(part, "FRACTURE");

            // Button IDs will match the pattern from old splint UI
    ...
    private String getButtonId(BodyPart part) {
        return switch (part) {
            case HEAD -> "HeadSplintButton";
            case TORSO -> "TorsoSplintButton";
            case LEFTARM -> "LeftArmSplintButton";
            case RIGHTARM -> "RightArmSplintButton";
            case LEFTLEG -> "LeftLegSplintButton";
            case RIGHTLEG -> "RightLegSplintButton";
            default -> "";
        };
    }
...
//                            String.format("§aSplinted %s — fracture treated!", part.getDisplayName())
//                    ));

//                    MortalWoundsPlugin.getPluginLogger().atInfo().log(
//                            String.format("[SPLINT] Applied splint to %s", part.getDisplayName())
//                    );
                }
                ...
                MortalWoundsPlugin.getPluginLogger().atWarning().log(
                        "Invalid body part in splint event: " + data.part
                );
            }
        }
    }
...
//DEBUG logging
        ...
        refreshTask = scheduler.scheduleAtFixedRate(() -> {
            //Debug logging
           // System.out.println("[DEBUG] SplintPageNew auto-refresh triggered!");
            try {
...
// Just send command updates - events are already bound from build()
    ...
    private boolean consumeSplint(Player player) {
        ...
        for (ItemContainer container : containers) {
            ...
            for (short slot = 0; slot < container.getCapacity(); slot++) {
                ...
                if (stack != null && !stack.isEmpty()
                        && "Splint_Crude_Fracture".equals(stack.getItem().getId())) {
    ...
    protected void close() {
        super.close();
        SplintSelectionUIRegistry.unregister(playerRef);
        if (refreshTask != null) {
            refreshTask.cancel(false);
        }
    }
    ...
    public static class SplintEventData {
        public String part;

        public static final BuilderCodec<SplintEventData> CODEC;

        static {
            CODEC = BuilderCodec.builder(SplintEventData.class, SplintEventData::new)
