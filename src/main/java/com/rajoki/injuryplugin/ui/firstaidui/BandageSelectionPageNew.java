...
// Bandage UI buttons/labels interactions, info, etc

public class BandageSelectionPageNew extends InteractiveCustomUIPage<BandageSelectionPageNew.BandageEventData> {

    private static final ScheduledExecutorService scheduler;

    static {
        scheduler = Executors.newScheduledThreadPool(1);
//        System.out.println("[DEBUG] BandagePageNew scheduler created: " + scheduler);
    }
    private ScheduledFuture<?> refreshTask;
    private final PlayerRef playerRef;

    public BandageSelectionPageNew(@Nonnull PlayerRef playerRef) {
        super(playerRef, CustomPageLifetime.CanDismissOrCloseThroughInteraction, BandageEventData.CODEC);
    ...
    public void build(@Nonnull Ref<EntityStore> ref,
        ...
        BandageSelectionUIRegistry.register(playerRef, this);

        uiCommandBuilder.append("BandageSelectionNew.ui");

        BodyPartComponent bodyPartData = store.getComponent(ref,
                MortalWoundsPlugin.getInstance().getBodyPartComponentType());

        if (bodyPartData != null) {
            // Update all visual elements (same as BodyPartStatsUI)
            ...
            // Update heavy bleed icons
            updateHeavyBleedIcons(uiCommandBuilder, bodyPartData);

            // NEW: Bind button events for bandaging
            bindBandageButtons(uiEventBuilder, bodyPartData);

            // NEW: Disable buttons for parts without fractures
    ...
    private void bindBandageButtons(UIEventBuilder eventBuilder, BodyPartComponent bodyPartData) {
        // Add buttons for each body part with bleeds
        for (BodyPart part : UI_PARTS) {
            ...
            // Button IDs will match the pattern from old bandage UI
    ...
    private String getButtonId(BodyPart part) {
        return switch (part) {
            case HEAD -> "HeadBandageButton";
            case TORSO -> "TorsoBandageButton";
            case LEFTARM -> "LeftArmBandageButton";
            case RIGHTARM -> "RightArmBandageButton";
            case LEFTLEG -> "LeftLegBandageButton";
            case RIGHTLEG -> "RightLegBandageButton";
            default -> ""; //
        };
    }
    ...
    public void handleDataEvent(@Nonnull Ref<EntityStore> ref,
        ...
        if (data.part != null) {
            try {
                ...
                if (bodyPartComp != null && (bodyPartComp.hasBodyPartEffect(part, "BLEED") ||
...
//                            String.format("§aBandaged %s — bleed treated!", part.getDisplayName())
//                    ));

//                    MortalWoundsPlugin.getPluginLogger().atInfo().log(
//                            String.format("[BANDAGE] Applied bandage to %s", part.getDisplayName())
//                    );
                }
                ...
                MortalWoundsPlugin.getPluginLogger().atWarning().log(
                        "Invalid body part in bandage event: " + data.part
                );
            }
        }
    }
    ...
    private void startAutoRefresh(Ref<EntityStore> ref, Store<EntityStore> store) {
        if (refreshTask != null) {
            //Debug Logging
            //System.out.println("[DEBUG] Cancelling existing refresh task");

            refreshTask.cancel(false);
        }
//DEBUG logging
        //System.out.println("[DEBUG] Creating new refresh task...");

        refreshTask = scheduler.scheduleAtFixedRate(() -> {
            //Debug logging
            //System.out.println("[DEBUG] BandagePageNew auto-refresh triggered!");
            try {
                ...
                if (player != null && !player.wasRemoved()) {
                    ...
                    if (bodyPartData != null) {
                       ...
                       // System.out.println("[DEBUG] BandagePageNew auto-refresh triggered");
                        ...
                        bindBandageButtons(eventBuilder, bodyPartData);
                        // OLD
                        // enableButtonsForBleedingParts(commandBuilder, bodyPartData);
    ...
    private boolean consumeBandage(Player player) {
        ...
        for (ItemContainer container : containers) {
            ...
            for (short slot = 0; slot < container.getCapacity(); slot++) {
                ...
                if (stack != null && !stack.isEmpty()
                        && "Bandage_Crude_Bleed".equals(stack.getItem().getId())) {
    ...
    public void refresh(Ref<EntityStore> ref, Store<EntityStore> store) {
        ...
        if (data != null) {
            ...
            bindBandageButtons(events, data);
    ...
    protected void close() {
        super.close();
        BandageSelectionUIRegistry.unregister(playerRef);
        if (refreshTask != null) {
            refreshTask.cancel(false);
        }
    }
    ...
    public static class BandageEventData {
        public String part;

        public static final BuilderCodec<BandageEventData> CODEC;

        static {
            CODEC = BuilderCodec.builder(BandageEventData.class, BandageEventData::new)
    ...
    private void enableButtonsForBleedingParts(UICommandBuilder builder,
                                               BodyPartComponent data) {
        for (BodyPart part : UI_PARTS) {
            boolean hasBleed = data.hasBodyPartEffect(part, "BLEED");
            boolean hasHeavyBleed = data.hasBodyPartEffect(part, "HEAVY_BLEED");

            if (hasBleed || hasHeavyBleed) {
                builder.set("#" + getButtonId(part) + ".Disabled", false);
            }
        }
    }
}